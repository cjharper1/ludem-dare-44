...
## \package Waf.Dependency.RunsAfter
## Add a runs-after dependency between projects. If project A depends on project B,
## then all outputs of A must be written before any task in project B is
## executed.
##
## The following example shows how to add a runs-after dependency between two
## projects.
##
## \code
##     bld(
...
## \param[in,out] project - The dependencies are specified by the 'runs_after'
## attribute. A dependency is added to each of the project's tasks.
@feature('*')
@after_method('process_rule')
@after_method('apply_link')
@after_method('CompileDotNetProject')
def AddRunsAfterDependencies(project):
    # GET THE PROJECT DEPENDENCIES.
    dependency_projects = []
    dependency_names = Utils.to_list(getattr(project, 'runs_after', []))
    ...
    for dependency_project in dependency_projects:
        for dependency_task in dependency_project.tasks:
            for task in project.tasks:
               task.set_run_after(dependency_task)
